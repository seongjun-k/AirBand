...
PyQt5 기반의 다크 테마 UI는 현재 모드, 인식된 손 위치, 재생 중인 음을 실시간으로 시각화하며, 카메라 화면 상에 직접 건반 구획과 드럼 패드 가이드라인을 매끄럽게 오버레이합니다. 피아노 모드와 드럼 모드는 GPIO 22에 연결된 단일 터치 스위치로 순환 전환하며, 로터리 엔코더로 옥타브와 감도를 조절할 수 있습니다. PIR 인체 감지 센서를 통해 일정 시간 사용이 없으면 자동으로 절전 모드로 전환되어 리소스를 아낍니다.

---

## 주요 기능

- **피아노 모드 (1옥타브 도~도)** — X축 8등분 건반 매핑으로 도(`C3`)부터 높은 도(`C4`)까지의 음계를 직관적으로 연주 (검지 Y 좌표로 볼륨 제어)
...
│   ├── piano_mode.py        # 피아노 1옥타브 음계 매핑 및 트리거 처리기
...
카메라 프레임 → MediaPipe 손 인식 → 검지 끝 좌표 추출
  ...
  └─ 트리거 모드(strike: 타격, continuous: 연속, pinch: 맞닿음)에 따라 소리 재생
```
...
터치 스위치    → mode_toggle        → 단일 터치 핀(GPIO 22) 입력 시 피아노 ↔ 드럼 교차 토글
로터리 엔코더  → encoder_rotated    → 옥타브(피아노) / 감도(드럼) / 볼륨 조절
...
| 터치 센서 | GPIO 22 | 피아노 🎹 ↔ 드럼 🥁 모드 토글 (단일 핀) |
