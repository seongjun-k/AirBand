...
PyQt5 기반의 GUI는 현재 모드, 인식된 손 위치, 재생 중인 음을 실시간으로 시각화합니다. 피아노 모드, 테레민 모드, 드럼 모드는 GPIO에 연결된 터치 스위치로 순환 전환하며, 로터리 엔코더로 옥타브와 감도, 볼륨을 물리적으로 조절할 수 있습니다. PIR 인체 감지 센서를 통해 일정 시간 사용이 없으면 자동으로 절전 모드로 전환되고 움직임이 감지되면 자동으로 절전이 해제됩니다.
...
- **테레민 모드** — 2D 평면 상의 미세한 움직임을 감지하여 도(`C3`)부터 높은 도(`C4`)까지의 음계를 테레민 악기처럼 지속적으로 연주 (검지 Y 좌표로 볼륨 제어)
...
AudioEngine (hardware/audio_engine.py)
  ...
  ├─ play_theremin(note, octave, vol)  → numpy 비브라토 Sine파 합성 후 pygame.mixer 즉시 재생
  └─ play_drum(pad, velocity)          → numpy Sine/Noise 합성 후 pygame.mixer 즉시 재생 (kick/snare/hihat/clap 자체 합성)
```
...
| `CAM_FPS` | `30` | 목표 FPS |
| `CAMERA_FLIP_MODE` | `0` | 카메라 플립 처리 (1: 좌우, 0: 상하, -1: 상하좌우, None: 반전없음) |
| `MP_MAX_NUM_HANDS` | `2` | 인식할 최대 손 개수 |
| `MP_TRACK_SKIP_FRAMES` | `1` | 추론 스킵 프레임 수 (0: 스킵 없음, 1: 1프레임 스킵) |
| `BASE_OCTAVE` | `3` | 피아노/테레민 기본 옥타브 |
| `PIANO_TRIGGER_MODE` | `'pinch'` | 피아노 트리거 방식 (`strike` / `pinch` / `continuous`) |
