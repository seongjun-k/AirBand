...
- 🖐️ **손 인식** — MediaPipe Hands Lite 모델로 최대 2개 손, 30 FPS 실시간 추적
- 🔌 **하드웨어 연동** — PIR 인체 감지 센서(자동 절전), 터치 스위치(모드 전환), 로터리 엔코더(옥타브/감도 조절)
- 🖥️ **풀스크린 UI** — PyQt5 기반 다크 테마 인터페이스
...
| 터치 스위치 — 피아노 | GPIO 27 | 피아노 모드 전환 |
| 터치 스위치 — 드럼 | GPIO 22 | 드럼 모드 전환 |
...
4. 브랜치를 Push 합니다. (`git push origin feat/new-feature`)
